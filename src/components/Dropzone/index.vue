...
export default class extends Vue {
  ...
  private dropzoneSending(file: File, xhr: XMLHttpRequest, formData: FormData) {
    const keys = this.header ? Object.keys(this.header) : []
    keys.length > 0 && keys.forEach(key => xhr.setRequestHeader(key, this.header[key]))
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private dropzoneError(file: File, response: any) {
    this.$message({ message: 'Upload error', type: 'error' })
    // this.$emit('dropzone-success', file, response)
  }
  ...
  mounted() {
    ...
    // ;(this.$refs.myVueDropzone as any).confirm = () => {
    //   console.log('upload')
    // }
  }
}
